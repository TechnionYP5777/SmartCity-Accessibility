...
public class LocationManager {
	...
	/**
	 * If the location saved it to the callback function If it's not saved
	...
	public static List<LatLng> getNonAccessibleLocationsInRadius(Location source, Location destination,
		...
		List<Location> returnMe = FilterToAllAbove(loc, 1, accessibilityThreshold); // '1'
																					// will
																					// be
																					// changed
																					// in
	...
	public static Location getLocation(LatLng point, Location.LocationTypes type, Location.LocationSubTypes subtype) {
			...
			public void done(List<ParseObject> arg0, ParseException arg1) {
				if (arg1 == null && arg0 != null) {
					for (ParseObject obj : arg0) {
						...
						reviews.add(new Review(new Location(point, type, subtype), obj.getInt("rating"),
		...
		// System.out.println("omg I am a stub? what does that mean!!!! what am
		// I??? ahhhhh");
		return new Location(point, type, subtype, reviews);
	}

	/**
	 * return list of location that belong to the point. since the reviews
	 * belong to a LatLng point (as we save only the coordinates) the list of
	 * reviews will return to all the locations.
	 * 
	 * @param point
	 * @param o
	 */
	public static void getLocation(LatLng point, LocationListCallback o) {
		...
		FindCallback<ParseObject> callBackR = new FindCallback<ParseObject>() {
			...
			public void done(List<ParseObject> arg0, ParseException arg1) {
				if (arg1 == null && arg0 != null) {
					for (ParseObject obj : arg0) {
						// reviews.add(new Review(l, r, c, u)) will be complete
						// after issue #124 will close
					}
